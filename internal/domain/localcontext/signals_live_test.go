...
// TestLiveNewsStaleWhileRevalidate makes REAL calls to GDELT. Keyless and
// unbilled, but slow. Skipped unless LOCI_LIVE_WEATHER=1.
//
// Run: LOCI_LIVE_WEATHER=1 go test ./internal/domain/localcontext/ -run TestLiveNews -v -timeout 5m
//
// It asserts only what this package controls — that the cold call is instant,
// that nothing surfaces as an error, and that anything returned obeys the
// severity rule. Whether GDELT actually answers is *reported, not asserted*: it
// took 23-26s when this source was written and failed to answer at all within
// three minutes on the next run, and a third party that flaky must not be able
// to fail this suite. That instability is itself the argument for
// GDELT_ENABLED=false.
func TestLiveNewsStaleWhileRevalidate(t *testing.T) {
	if os.Getenv("LOCI_LIVE_WEATHER") != "1" {
		t.Skip("set LOCI_LIVE_WEATHER=1 to run the live GDELT test")
	}

	// The shared client times out at 8s, which GDELT has never once beaten.
	s := NewNewsSource("", httpxClientForNews(), nil, nil)
	req := SignalRequest{
		CountryCode: "FR",
	...
	// The contract that makes a 26s dependency usable at all.
	started := time.Now()
	first, err := s.Fetch(context.Background(), req)
	firstCall := time.Since(started)
	if err != nil {
		t.Fatalf("Fetch must never surface an error, got %v", err)
	}
	if firstCall > 2*time.Second {
		t.Fatalf("the cold call must not block on GDELT, took %v", firstCall)
	}
	if len(first) != 0 {
		t.Errorf("a cold cache should yield nothing, got %d", len(first))
	}
	t.Logf("cold call: %d alerts in %v (must be instant — this is the contract)", len(first), firstCall)

	// Give the detached refresh a chance, but do not require it.
	...
	if !landed {
		t.Logf("GDELT did not answer within %v — reported, not failed. "+
			"This is exactly why the source is off by default and never blocks a request.",
	...
	t.Logf("refresh landed after ~%v; %d alerts survived filtering",
		time.Since(started).Round(time.Second), len(got))
	for _, a := range got {
		if a.Severity != SeverityMinor {
			t.Errorf("news must always be minor, got %v for %q", a.Severity, a.Title)
		}
		if a.Source != SourceNews {
			t.Errorf("source: got %q", a.Source)
		}
		if a.Located() {
			t.Errorf("a news alert must not be located: %q", a.Title)
		}
		t.Logf("  %s", a.Title)
	}
}
